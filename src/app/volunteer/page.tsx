...
type ScreenState = "home" | "alert" | "navigating" | "arrived" | "resolved";
...
export default function VolunteerPage() {
  ...
  async function handleAccept() {
    ...
    await broadcast({ type: "VOLUNTEER_ACCEPTED", volunteer: "د. أحمد الشهري" });
  }
  ...
  async function handleArrived() {
    ...
    setScreen("arrived");
    await broadcast({ type: "VOLUNTEER_ARRIVED" });
  }

  async function handleOutcome(type: OutcomeType) {
    setOutcome(type);
    setScreen("resolved");
    await broadcast({ type: "CASE_RESOLVED", outcome: type });
  ...
  return (
    <div className="min-h-screen bg-gray-200 flex items-center justify-center p-4" dir="rtl">
      ...
      <div className="relative w-[375px] h-[812px] bg-black rounded-[50px] shadow-2xl overflow-hidden border-4 border-gray-800 flex flex-col">
        ...
        {/* Status bar */}
        <div className="flex items-center justify-between px-8 pt-3 pb-1 bg-black text-white text-xs z-40 flex-shrink-0" style={{ paddingTop: "28px" }}>
      ...
      <div className="px-4 py-3 space-y-3 flex-1">
        ...
        <div className="bg-white rounded-2xl p-4 shadow-sm">
          ...
          <div className="h-1.5 bg-gray-100 rounded-full">
            <div className="h-full bg-saudi-green rounded-full" style={{ width: "75%" }} />
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl p-4 shadow-sm">
          ...
          <div className="grid grid-cols-5 gap-1 text-center">
            {[
              ...
              { name: "العصر", time: "15:32" },
              { name: "المغرب", time: "18:47" },
      ...
      <div className="bg-white/15 backdrop-blur-sm rounded-2xl p-5 w-full border border-white/20">
        <div className="text-white space-y-3">
          ...
          <div className="border-t border-white/20 pt-3 grid grid-cols-2 gap-3">
            <div>
              <p className="text-red-300 text-xs mb-0.5">وصول الإسعاف</p>
              <p className="font-bold text-lg">١١ دقيقة</p>
            </div>
            <div>
              <p className="text-red-300 text-xs mb-0.5">تأهيلك</p>
              <p className="font-bold text-sm">طبيب طوارئ</p>
            </div>
          </div>
        </div>
      </div>
...
function ResolvedScreen({ outcome, responseTime }: { outcome: OutcomeType | null; responseTime: number }) {
  const outcomeConfig = {
    stable: { icon: "🟢", text: "الحالة مستقرة", color: "text-green-600", bg: "bg-green-50" },
    transport: { icon: "🟡", text: "في طريقه للمستشفى", color: "text-yellow-600", bg: "bg-yellow-50" },
    critical: { icon: "🔴", text: "دعم فوري مطلوب", color: "text-red-600", bg: "bg-red-50" },
  };

  const config = outcome ? outcomeConfig[outcome] : outcomeConfig.stable;
  ...
  return (
    <div className="h-full bg-gradient-to-b from-saudi-green to-saudi-green-dark flex flex-col items-center justify-center px-6 text-center text-white">
      ...
      <div className="bg-white/15 rounded-2xl p-5 w-full mb-6 space-y-3 border border-white/20">
        ...
        <div className="flex items-center justify-between">
          <span className="text-green-200 text-sm">الإسعاف الأول</span>
          <span className="font-bold text-base">تم التوفير</span>
        </div>
        <div className="border-t border-white/20" />
        <div className="flex items-center justify-between">
          <span className="text-green-200 text-sm">نتيجة الحالة</span>
          <span className={`font-bold text-sm bg-white/20 rounded-full px-3 py-0.5`}>
